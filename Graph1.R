...
# Data
load(file='./data/ba.Rdata')
df1
attach(df1)
#scatter Plot
?plot
plot(adtv, sales)
plot(x=adtv, y=adweb, pch=10, type='p', col='green') #pch is plotting symbol fixed for specific color
plot(x=adtv, y=adweb, pch=15, type='p', col='yellow')
abline(lm(adweb ~ adtv)) #regression line
library(car)
car::scatterplot(sales ~ coy, data=df1, legend.columns=T) # coy is company
...
nvars = c('sales', 'adtv', 'adweb') #corelation between 3 variable
...
#Histogram
hist(df1$sales)
hist(df1$sales, freq=F, col=1:12, breaks=12)# 12 value can be changed to reduce or increase number of rectangle
?hist
